        ...
        for file in segFiles:
            ...
            for i, ts in enumerate(seg_images[:,0]):
                ...
                if num > 0:                        
                    ...
                    if num == 1:
                        if prior_results:
                            # Interpolate bounding boxes from prior box only if we are sure that they are the same
                            euclid_dist = math.sqrt((prior_results[0][0] - results[0][0])**2 + (prior_results[0][1] - results[0][1])**2)
                            if euclid_dist < 90:
                                lin_x = (results[0][0] - prior_results[0][0])/interp_factor
                                lin_y = (results[0][1] - prior_results[0][1])/interp_factor
                                lin_w = (results[0][2] - prior_results[0][2])/interp_factor
                                lin_h = (results[0][3] - prior_results[0][3])/interp_factor
                                lin_ts = (ts - prior_ts )//interp_factor
                                for j in range(0, interp_factor-1):
                                    bbox = [(int(lin_x*(j+1)+prior_results[0][0]),
                                             int(lin_y*(j+1)+prior_results[0][1]),
                                             int(lin_w*(j+1)+prior_results[0][2]),
                                             int(lin_h*(j+1)+prior_results[0][3]), 0, 1, self.trackid)]
                                    list_res = list(bbox[0])
                                    list_res.insert(0, lin_ts*(j+1)+prior_ts)
                                    res = tuple(list_res)
                        ...
                        list_res.insert(6, self.trackid)
